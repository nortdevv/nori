 ...
 * Cada test que crea proyecto borra por API al terminar (evita basura "E2E …" en el dashboard).
 */
...
test.describe("E01 E04–E08 (autenticado)", () => {
  ...
  test("E05: crear proyecto y llegar al chat", async ({ page }) => {
    ...
    try {
      projectId = await createProjectViaUi(page, `E2E Proyecto ${Date.now()}`);
  ...
  test("E08: Generar Documento abre vista previa o error usable", async ({
    ...
    let projectId: string | null = null;
    try {
      projectId = await createProjectViaUi(page, `E2E Doc ${Date.now()}`);
      await expect(page).toHaveURL(/\/chat\//);
      ...
      await expect(overlay.or(errorText).first()).toBeVisible({
        timeout: 90_000,
      });
    } finally {
      await deleteTestProject(projectId);
